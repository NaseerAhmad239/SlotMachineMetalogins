...
namespace Mkey
{
    ...
    public class SlotController : MonoBehaviour
    {
        ...
        private void RunSlots()
        {
            ...
            {
                    MessageManager.Instance.ShowPanel("You have no money");
                controls.ResetAutoSpinsMode();

                // MGUI.ShowMessage(null, "You have no money.", 1.5f, null);
                return;
            }
            if (SlotPlayer.Instance.Coins < Constants.StopAtCash)
            {
                MessageManager.Instance.ShowPanel("Money Reached to Lowest value that you set");
                controls.ResetAutoSpinsMode();
        ...
        private IEnumerator RunSlotsAsync()
        {
            ...
            if (controls.Auto)
            {
                if (controls.AutoSpinTotalCount != 0)
                {
                    controls.AutoSpinTotalCount--;
                    controls.IncAutoSpinsCounter();
                }
                else
                {
                    controls.ResetAutoSpinsMode();
        ...
        private void RotateSlots(Action rotCallBack)
        {
            ...
            if (debugPredictSymbols)
                for (int i = 0; i < rands.Length; i++)
                {
                    ...
                    for (int r = 0; r < slotGroupsBeh[i].RayCasters.Length; r++)
                    {
                        ...
                        newsymbols.Add(sName);
                    }
                }
            #endregion prediction


            //ModeType.Instance.newsymbols = newsymbols;
            //ModeType.Instance.newsymbols.Reverse();


            //ModeType.Instance.Row = false;
            //for (int j = 0; j < ModeType.Instance.MatchPattern.Length; j++)
            //{
            //    //ModeType.Instance.Row = false;
            //    ModeType.Instance.predictNumber = new();
            //    if (HasThreeOrMoreConsecutiveValues(ModeType.Instance.MatchPattern[j].Pattern)==true)
            //    {
            //        ModeType.Instance.Row = true;
            //        for (int i = 0; i < ModeType.Instance.MatchPattern[j].Pattern.Length; i++)
            //        {
            //            ModeType.Instance.predictNumber.Add(ModeType.Instance.predictint[ModeType.Instance.MatchPattern[j].Pattern[i]]);
            //        }

            //        //ModeType.Instance.predictNumber.Reverse();
            //        //break;
            //    }
            //    else
            //    {
            //        for (int i = 0; i < ModeType.Instance.MatchPattern[j].Pattern.Length; i++)
            //        {
            //            ModeType.Instance.predictNumber.Add(0);
            //        }
            //    }
            //}
        ...
        //bool AreAllValuesEqual(int[] values)
        //{
        //    if (values.Length == 0)
        //        return true;
        ...
        //    {
        //        if (ModeType.Instance.newsymbols[value] != ModeType.Instance.newsymbols[firstValue])
        //            return false;
        //    }
        //    return true;
        //}

        //bool HasThreeOrMoreConsecutiveValues(int[] values)
        //{
        //    if (values.Length < 3)
        //        return false;

        //    int consecutiveCount = 1;
        //    string repeatingValue = null; // To store the repeating value
        //    for (int i = 1; i < values.Length; i++)
        //    {
        //        if (ModeType.Instance.newsymbols[values[i]] == ModeType.Instance.newsymbols[values[i - 1]])
        //        {
        //            consecutiveCount++;
        //            repeatingValue = ModeType.Instance.newsymbols[values[i]];
        //            if (consecutiveCount >= 3)
        //            {
        //                //Debug.LogError($"Value '{repeatingValue}' is repeating {consecutiveCount} times consecutively.");
        //                return true;
        //            }
        //        }
        //        else
        ...
        public void MessageForFreeSpin(int Total)
        {
            MessageManager.Instance.ShowPanel($"You Got {Total} Free Spins");
        }
