...
namespace Mkey
{
    public class GradientLineRenderer : SlotLineRenderer
    {
        ...
        public SpriteRenderer[] lSRs;
        public List<SpriteRenderer> dSRs;
        ...
        internal override void SetLineVisible(bool visible)
        {
            //if (visible)
            //{
            //    List<int> currentVal = GetComponentInParent<LineBehavior>().CurrentNumber;
            //    int commonVal = GetComponentInParent<LineBehavior>().FindMostCommonNumber(GetComponentInParent<LineBehavior>().CurrentNumber.ToArray());
            ...
            //        {
            //            if (dSRs[i] != null && currentVal[i] == commonVal)
            //            {
            //                dSRs[i].gameObject.SetActive(visible);
            //            }
            //        }
            //    }

            //    for (int i = 0; i < dSRs.Count - 1; i++)
            //    {
            //        if (dSRs[i] != null && dSRs[i + 1] != null)
            //        {
            //            if (dSRs[i].gameObject.activeSelf && dSRs[i + 1].gameObject.activeSelf)
            //            {
            //                int lineIndex = i; // Assuming the line index corresponds to the dot index
            //                if (lineIndex < lSRs.Length && lSRs[lineIndex] != null)
            //                {
            //                    lSRs[lineIndex].gameObject.SetActive(true);
            //                }
            //            }
            //        }
            //    }
