...
namespace Mkey
{
    public class WinController : MonoBehaviour
    {
        ...
        [SerializeField]
        private float lineWinShowTime = 2f;
        ...
        {
            lineWinShowTime = (lineWinShowTime < 2) ? 2 : lineWinShowTime;
        }
        ...
        /// </summary>
        internal void SearchWinSymbols(/*int predict1, int predict2, int predict3*/)
        {
            foreach (var lb in linesController.Lines)
            {
                if (lb.IsSelected)
                {
                    lb.FindWin(payTable /*, predict1,predict2,predict3*/);
                }
            }
        ...
        public int GetWinCoins()
        {
            ...
            return (int)((res /** slot.controls.TotalBet*/)/**ModeType.Instance.ModeBetMultiplyer*/);
        }
        ...
        public int GetLineWinCoinsCalc()
        {
            ...
            foreach (LineBehavior lB in linesController.Lines)
            {
                ...
                {
                    Debug.LogError($"{res} {lB.winCalc.Pay} {lB.gameObject.name}");
                    res += lB.winCalc.Pay;
                    Debug.LogError($"{res} ");
