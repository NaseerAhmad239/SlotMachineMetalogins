      ...
      //  [SerializeField]
       ...
       // [SerializeField]
        ...
        public int TotalBet
        {
            ...
            get {
                print("LineBes "+LineBet);
                print("selectedlinescount  "+SelectedLinesCount);
                print("holdmultipler "+HoldMultipler);



                return LineBet * ModeType.Instance.BetIcrementAndDecrementValue * HoldMultipler; 
        ...
        private IEnumerator StartC()
        {
            ...
            if (balanceAmountText) balanceTween = new TweenIntValue(balanceAmountText.gameObject, MPlayer.Coins, 1, 2, true, (b) => { if (this && balanceAmountText) balanceAmountText.text = (b > 0) ?FormatMoney(b) : "0"; });
        ...
        private void OnValidate()
        {
            ...
            maxLineBet = Math.Max(1, ModeType.Instance.maxLineBet);
            defLineBet = Math.Max(1, defLineBet);
            defLineBet = Mathf.Min(defLineBet, ModeType.Instance.maxLineBet);
        ...
        {
            if (MPlayer && balanceAmountText) balanceAmountText.text =FormatMoney(MPlayer.Coins);
        }
        ...
        private void RefreshSpins()
        {
            ...
            if (FreeSpinCountText) FreeSpinCountText.text = (FreeSpins > 0) ? FreeSpins.ToString() : "0";
        }
        ...
        public void SetWinInfo(float coins)
        {
        ...
        {
            AutoSpinTotalCount = Constants.TotalLoop;
            if (Auto) { ResetAutoSpinsMode(); return; }
		...
		private void ChangeFreeSpinsHandler(int newFreeSpinsCount)
		{
			...
			{
                if (FreeSpinText) FreeSpinText.text = (FreeSpins > 0) ? "Free Spins" : "";
                if (FreeSpinCountText) FreeSpinCountText.text = (newFreeSpinsCount > 0) ? newFreeSpinsCount.ToString() : "0";
			}
		}
        ...
        private void ChangeBalanceHandler(int newBalance)
        {
            ...
            //else
            //{
                if (balanceAmountText) balanceAmountText.text = (newBalance > 0) ? FormatMoney(newBalance) : "0";
        ...
        public void SetLineBet(int count)
        {
            ...
            //count += count;
            if (count > ModeType.Instance.maxLineBet)
            {
                count = defLineBet;
            }
            else if(count < defLineBet)
            {
                count = ModeType.Instance.maxLineBet;
            }
            bool changed = (LineBet != count);
            LineBet = count;
            Debug.LogError($"{LineBet} ");
        ...
        {
            SetLineBet(ModeType.Instance.maxLineBet);
        }
        ...
        {
            Debug.LogError("AutoSpin Chal raha hai");
