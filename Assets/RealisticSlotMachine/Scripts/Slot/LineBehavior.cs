...
namespace Mkey
{
    public class LineBehavior : MonoBehaviour
    {
        ...
        internal float WonCoins
        {
            get
            {
                //Debug.LogError($" {win.Pay} ok {slot.controls.TotalBet}");
                return (win == null) ? 0 : (win.Pay)/* * slot.controls.TotalBet)/**ModeType.Instance.ModeBetMultiplyer*/;
            }
        }
        ...
        /// </summary>
        internal void FindWin(List<PayLine> payTable/*, int predict1, int predict2, int predict3*/)
        {
            win = null;
            WinData winTemp = null;
            //Debug.LogError("ShortCut");
            foreach (var item in payTable)
            {
                // find max win
                winTemp = GetPayLineWin(item /*predict1, predict2, predict3*/);
                if (winTemp != null)
                {
                    if (win == null)
                    {
                        win = winTemp;
                        //Debug.LogError($" aye haye {win.Pay} ");
        ...
        /// <returns></returns>
        public List<SlotSymbol> winnSymbols = new List<SlotSymbol>();
        ...
        private void Start()
        {
            ...
            {
                Index.Add(rayCasters[i].index);
            }

            for (int i = 0; i < ModeType.Instance.MatchPattern.Length; i++)
            {
                if (AreArraysEqual<int>(Index.ToArray(), ModeType.Instance.MatchPattern[i].Pattern))
        ...
        bool AreArraysEqual<T>(T[] array1, T[] array2)
        {
        ...
        private WinData GetPayLineWin(PayLine payLine)
        {
            if (isCheckable)
            {
                ...
                for (int i = 0; i < rayCasters.Length; i++)
                {
                    ...
                    else if (payLine.line[i] >= 0 && s.IconID == payLine.line[i]/*&& HasThreeOrMoreConsecutiveValues(CurrentNumber) == true && ModeType.Instance.Row == true*/)
                    {
                        winnSymbols.Add(s);
                    }
                    //Debug.LogError($" insaaan    {payLine.pay}");
                }
                ...
                //for (int i = 0; i < rayCasters.Length; i++)
                //{
                //    CurrentNumber.Add(rayCasters[i].GetSymbol().IconID);
                //}


                //if (HasThreeOrMoreConsecutiveValues(CurrentNumber) == true && ModeType.Instance.Row == true /*&& Checkpredictionfile(s.IconID)*/)
                //{
                //    int common = FindMostCommonNumber(CurrentNumber.ToArray());
                //    for (int i = 0; i < rayCasters.Length; i++)
                //    {
                ...
                //        //    winnSymbols.Add(s);
                //        //    //Debug.LogError("Not Working" + gameObject.name);
                //        //}
                ...
                //    return null;
                //}
                //Debug.LogError($" row    {payLine.pay}");
                ////ModeType.Instance.Row = false;
                //return new WinData(winnSymbols, payLine.freeSpins, payLine.pay, payLine.payMult, payLine.freeSpinsMult, payLine.LineEvent);
        ...
        //public bool Checkpredictionfile(int val)
        //{
        //    foreach (var s in ModeType.Instance.predictNumber)
        //    {
        //        {
        //            if (s != val)
        //            {
        ...
        //    return true;
        //}
        bool HasThreeOrMoreConsecutiveValues(List<int> values)
        {
            if (values.Count < 3)
                return false;

            int consecutiveCount = 1;
            int repeatingValue = values[0]; // To store the repeating value
            ...
            for (int i = 1; i < values.Count; i++)
            {
                ...
                {
                    consecutiveCount++;
                    repeatingValue = values[i]; // Update the repeating value
                    if (consecutiveCount >= 3)
                    {
                        Debug.Log($"Value '{repeatingValue}' is repeating {consecutiveCount} times consecutively.");
                        return true;
                    }
                }
                else
        ...
        public int FindMostCommonNumber(int[] array)
        {
            ...
            //Debug.LogError($"Most Common {mostCommon}  {maxCount}");

            return mostCommon;
        }
        //bool HasThreeOrMoreConsecutiveValues(int val)
        //{
        //    if (ModeType.Instance.predictNumber.Count < 3)
        //        return false;

        //    int consecutiveCount = 1;

        //    for (int i = 1; i < ModeType.Instance.predictNumber.Count; i++)
        //    {
        //        if (ModeType.Instance.predictNumber[i] == val && ModeType.Instance.predictNumber[i] == ModeType.Instance.predictNumber[i - 1])
        //        {
        //            consecutiveCount++;
        //            if (consecutiveCount >= 3)
        //            {
            ...
            foreach (var item in payTable)
            {
                ...
                if (winTemp != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Debug.LogError($"{winTemp.Pay} {winCalc.Pay} ");

                            winCalc = winTemp;
                            Debug.LogError($" {winCalc.Pay} ");
