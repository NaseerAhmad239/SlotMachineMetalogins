...
namespace Mkey
{
	public class BalanceGUIController : MonoBehaviour
	{
        ...
        private IEnumerator StartC()
        {
            ...
            if (balanceAmountText) balanceTween = new TweenIntValue(balanceAmountText.gameObject, MPlayer.Coins, 1, 3, true, (b) => { if (this && balanceAmountText) balanceAmountText.text = (b > 0) ?FormatMoney(b) : "0"; });
            RefreshBalance();
        }
        ...
        {
            if (balanceAmountText && MPlayer) balanceAmountText.text = (MPlayer.Coins > 0) ? FormatMoney(MPlayer.Coins) : "0";
        }
